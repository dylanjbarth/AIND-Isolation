"""
Used for quickly evaluating the effectiveness of various heuristic functions
against the alpha beta improved player.
"""
import itertools
import random
import warnings
...
def main():
    ...
    for i, f in enumerate(ALL_HEURISTICS_AS_SCORE_FCNS):
        print("{}: {}".format(i, f[1]))
    test_agents = [
        Agent(AlphaBetaPlayer(score_fn=f[0]), i) for i, f in enumerate(ALL_HEURISTICS_AS_SCORE_FCNS)
    ]
